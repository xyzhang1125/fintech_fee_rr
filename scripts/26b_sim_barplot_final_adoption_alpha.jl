...
# Function that calculates the cumulative adoption rate of the deadline group.
# Assume uniform distribution c over [mc-hc,mc+hc].
function adoption_rate(;
    ...
    if deadline
        for t = 1:T # Deadline group.
            cad = F(beta*delta*(alpha*y-rho_hat*ev[T+1-t])) # Conditional adoption rate.
            ad = ad+rem*cad # Update adoption rate.
            rem = rem*(1-cad)*rho # Update mass of remaining agents.
        end
    else 
        cad = F(beta*delta*(alpha*y-rho_hat*evend)) # Conditional adoption rate.
        for t = 1:T # No deadline group.
            ad = ad+rem*cad # Update adoption rate.
            rem = rem*(1-cad)*rho # Update mass of remaining agents.
        end
    end
...
# Function that tests whether final adoption rates increase with alpha.
# Note that there is no alpha in arguments.
# Return: 1= Pred holds; 0= no valid cases; -1= Pred does not hold.
function check_mono_alpha(;
    rho = 0.995,
    rho_hat = 0.995,
    beta = 0.8,
    beta_hat = 1,
    ...
    for alpha = LinRange(1, 0, 101) # Loop over alpha in a reverse order.
        cr = adoption_rate(;
            rho=rho, rho_hat=rho_hat, beta_hat=beta_hat, delta=delta,
            alpha=alpha, y=y, beta=beta, deadline=deadline
        ) # Obtain the current adoption difference.
        #println(y, '=', cr)
        if cr>pr+1e-14 # If difference narrows compared to the previous case.
...
function single_get_ratio(iter, beta_hat; deadline=true)
...
function get_ratio(beta_hat; deadline=true) 
    cnt1 = 0 # Count the number of tuples such that Prediction holds.
    ...
    tasks = map(chunks) do chunk
        Threads.@spawn single_get_ratio(chunk, beta_hat; deadline=deadline)
    end
...
for i = 0:M # i for rho, j for beta_hat.
    res_d[i+1] = get_ratio(i/M)
end


# Write out the CSV into the figures folder
writedlm(joinpath(fig_dir, "data_barplot_final_adoption_alpha.csv"), res_d)
