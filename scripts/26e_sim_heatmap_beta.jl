...
@memoize ThreadSafeDict function ev_iteration(
    ...
    F = x -> min(1, max(0, ((x-mc)/hc+1)/2)) # Cumulative distributive function.
...
function adoption_difference(;
    ...
    rem = 1.0 # Reset mass of remaining agents.
...
# Note that there is no beta in arguments.
# Return: 1= Pred holds; 0= no valid cases; -1= Pred does not hold.
function check_mono_beta(;
    rho = 0.995,
    rho_hat = 0.995,
    beta_hat = 1,
    delta = 0.99,
    ...
    for beta = (100:-1:1)/100 # Loop over beta in a reverse order.
        if beta>beta_hat # Do not consider cases where beta>beta_hat.
            continue
        end
        ...
        elseif cr<pr-1e-10 # If difference narrows compared to the previous case.
...
for TT = [1, 2, 6, 8] # Different deadline days.
    ...
    for i = 0:M # i for rho.
        ...
        for j = 0:M # j for beta_hat.
            ...
            for rho_hat = (0:100)/100 # Loop over rho_hat.
                cr = check_mono_beta(
                    rho_hat=rho_hat, 
                    rho=i/M, 
                    beta_hat=j/M,
            ...
            res[i+1,j+1] = cnt1/(cnt1+cntm1+1e-10) # Store ratios (1e-10 in case of both being zeros).
        end
    end

    # Export the matrix as CSV into the figures folder
    writedlm(
        joinpath(fig_dir, "data_heatmap_beta_t$(TT).csv"),
        res
    )
